...
import { calcDailyStats, formatCurrency, getCategoryInfo, getMonthName, MONTH_NAMES } from '../../utils/calculations'
...
export default function CalendarView() {
  const { transactions, currentMonth, currentYear } = useApp()
  ...
  function getDayStats(day) {
    return calcDailyStats(transactions, getDateStr(day))
  }
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto animate-fade-in pb-24 lg:pb-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
        ...
        <div className="grid grid-cols-7 border-b border-slate-200 dark:border-slate-700">
          {DAYS.map(d => (
            <div key={d} className="py-2 text-center text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase">
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
            ...
            const hasExpense = stats.expenses > 0
            ...
            return (
              ...
              >
                <span className={`text-xs font-semibold w-6 h-6 flex items-center justify-center rounded-full mb-1 ${
                  today_ ? 'bg-indigo-600 text-white' : 'text-slate-700 dark:text-slate-300'
                }`}>
                  {day}
                </span>
                <div className="space-y-0.5">
                  {hasIncome && (
                    <div className="text-[10px] font-medium text-green-600 dark:text-green-400 leading-tight truncate">
                      +{formatCurrency(stats.income)}
                    </div>
                  )}
                  {hasExpense && (
                    <div className="text-[10px] font-medium text-red-500 dark:text-red-400 leading-tight truncate">
      ...
      {selectedDay && selectedStats && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm animate-slide-up">
          <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
            <div>
              ...
              <p className="text-xs text-slate-400">
                ...
                {selectedStats.income > 0 && ` · +${formatCurrency(selectedStats.income)}`}
                {selectedStats.expenses > 0 && ` · −${formatCurrency(selectedStats.expenses)}`}
              </p>
            </div>
