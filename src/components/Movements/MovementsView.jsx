...
import { calcMonthlyStats, getCategoryInfo, formatDate } from '../../utils/calculations'
import MovementForm from './MovementForm'

export default function MovementsView() {
  const { transactions, deleteTransaction, currentMonth, currentYear, fmt, allCategories } = useApp()
  ...
  const groups = filtered.reduce((acc, tx) => {
    const d = tx.date;(acc[d] = acc[d] ?? []).push(tx); return acc
  }, {})
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto animate-fade-in pb-24 lg:pb-6">
      ...
      {sortedDates.length > 0 ? (
        ...
        <div className="text-center py-16 text-slate-400 dark:text-slate-500">
          ...
          <p className="text-sm mt-1">{search ? 'No hay resultados para tu búsqueda.' : 'Agrega tu primer movimiento.'}</p>
        </div>
      )}
