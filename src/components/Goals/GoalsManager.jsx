...
import { calcMonthlyStats, calcBudgetProgress, getCategoryInfo, getMonthName } from '../../utils/calculations'

export default function GoalsManager() {
  const { transactions, monthBudgets, monthGoal, currentMonth, currentYear, saveGoal,
          fmt, allExpenseCategories, allCategories } = useApp()
  ...
  // Initialize form when categories or goal changes
  useEffect(() => {
    const initial = allExpenseCategories.reduce((acc, c) => ({ ...acc, [c.id]: '' }), {})
  ...
  useEffect(() => {
    ...
    const map = {}
    allExpenseCategories.forEach(c => { map[c.id] = '' })
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">
                  {fmt(stats.balance)} / {fmt(monthGoal.savings_goal)}
                </span>
              </div>
              <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-700 ${stats.balance >= monthGoal.savings_goal ? 'bg-green-500' : 'bg-indigo-500'}`}
              ...
              {stats.balance >= monthGoal.savings_goal
                ...
                : <p className="text-xs text-slate-400 mt-0.5">Te faltan {fmt(monthGoal.savings_goal - stats.balance)}</p>
              }
            </div>
          )}
              ...
              <div key={b.id}>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <div className="text-right">
                    ...
                    {stats.income > 0 && <span className="text-xs text-slate-400 ml-2">({b.incomePct}% ingreso)</span>}
                  </div>
                </div>
                <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-700 ${b.isOver ? 'bg-red-500' : b.pct > 80 ? 'bg-amber-500' : 'bg-indigo-500'}`}
                    style={{ width: `${b.pct}%` }} />
                </div>
                {b.isOver
                  ? <p className="text-xs text-red-500 mt-0.5">Superado en {fmt(Math.abs(b.remaining))}</p>
                  : <p className="text-xs text-slate-400 mt-0.5">Disponible: {fmt(b.remaining)}</p>
                }
              </div>
            )
          })}
      ...
      <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-4 space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="number" min="0" step="0.01" value={savingsGoal}
              onChange={e => setSavingsGoal(e.target.value)} placeholder="ej: 500.00"
          ...
          <p className="text-xs text-slate-400 mt-1">
            Se recomienda el 20% de tus ingresos{stats.income > 0 ? ` (${fmt(stats.income * 0.2)})` : ''}.
          </p>
        </div>
        ...
        <button type="submit" disabled={saving}
          ...
          {saving ? <><Loader2 size={16} className="animate-spin" /> Guardando...</> : <><Save size={16} /> Guardar Objetivos</>}
        </button>
      </form>
    </div>
  )
