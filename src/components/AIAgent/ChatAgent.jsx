  ...
  return (
    <div className={`flex gap-3 ${isAI ? '' : 'flex-row-reverse'}`}>
      ...
      <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
        isAI ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'
      }`}>
      ...
      <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
        ...
        <p className={`text-[10px] mt-1.5 ${isAI ? 'text-slate-400' : 'text-indigo-200'}`}>
          {msg.time}
        </p>
      </div>
    </div>
...
// Minimal markdown renderer (bold, headers, lists, tables)
...
export default function ChatAgent() {
  ...
  async function send(text) {
    ...
    // Simulated async think time (150–400ms)
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
        ...
        <button onClick={reset} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-colors" title="Reiniciar chat">
          <RotateCcw size={16} />
        </button>
      </div>
